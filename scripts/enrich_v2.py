...
def parse_args():
    ...
    ap.add_argument("--id", dest="job_id", help="Process only this job _id (string)")
    ap.add_argument("--limit", type=int, default=None, help="Override batch size for a single run")
...
def build_selector():
    ...
    if ARGS.job_id:
        # Respect staleness rule even for --id to allow one-shot behavior
...
def select_jobs(limit: int):
    q = build_selector()
    cur = jobs_col.find(q, projection=base_projection(), limit=limit)
    return list(cur)

def count_total_to_process():
    q = build_selector()
    return jobs_col.count_documents(q)
...
def main():
    total_to_process = count_total_to_process()
    if ARGS.job_id:
        # Force total to 1 for cleaner progress when targeting a single doc
        total_to_process = min(total_to_process, 1)

    processed = 0
    while True:
        left = total_to_process - processed if total_to_process > 0 else None
        batch_limit = ARGS.limit or min(BATCH_SIZE, left) if left is not None else (ARGS.limit or BATCH_SIZE)
        ...
        if not ARGS.no_progress:
            line = render_progress(processed, total_to_process)
            # carriage return (no newline), flush to update in place
            sys.stdout.write("\r" + line)
            sys.stdout.flush()

        # If we’re in single-id mode, stop after first pass
        if ARGS.job_id:
            break
    ...
    if not ARGS.no_progress:
        sys.stdout.write("\n")
    print(json.dumps({"ok": True, "processed": processed, "total": total_to_process, "db": DB_NAME}, indent=2))
