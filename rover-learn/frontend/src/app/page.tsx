...
export default function LivePage() {
  ...
  useEffect(() => {
    if (!session) return;
    const fetchStatus = () => {
      getSessionStatus(session._id).then(setStatus).catch(() => {});
    };
    fetchStatus();
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', top: '0.5rem', right: '0.5rem', fontSize: '0.8rem' }}>
        {status && (
          <>
            Capture: {status.capture} • Last ingest: {status.lastIngestSec ? Math.round(status.lastIngestSec) : '–'}s • Latency:{' '}
            {Math.round((status.metrics?.asr_ms || 0) + (status.metrics?.mt_ms || 0))} ms
          </>
        )}
      </div>
    </div>
