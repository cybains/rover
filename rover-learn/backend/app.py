...
SESSION_METRICS: Dict[str, Dict[str, float | int]] = defaultdict(
    lambda: {"asr_ms": 0.0, "mt_ms": 0.0, "asr_n": 0, "mt_n": 0, "dropped": 0}
)
...
def _update_metrics(
    ...
    m = SESSION_METRICS[session_id]
    if asr_ms is not None:
        n = m.get("asr_n", 0)
        m["asr_ms"] = (m.get("asr_ms", 0.0) * n + asr_ms) / (n + 1)
        m["asr_n"] = n + 1
    if mt_ms is not None:
        n = m.get("mt_n", 0)
        m["mt_ms"] = (m.get("mt_ms", 0.0) * n + mt_ms) / (n + 1)
        m["mt_n"] = n + 1
    if dropped:
        m["dropped"] = m.get("dropped", 0) + dropped
...
async def _watchdog():
    while True:
        ...
        now = datetime.utcnow()
        for sid, proc in list(CAPTURE_PROC.items()):
            if proc.poll() is None:
                last = LAST_INGEST_AT.get(sid)
                if last and (now - last).total_seconds() > 30:
                    await _stop_capture(sid)
                    await _start_capture(sid)
...
async def heartbeat(payload: dict = Body(...)):
    ...
    dropped = int((payload or {}).get("dropped", 0) or 0)
    if dropped:
        _update_metrics(session_id, dropped=dropped)
    return {"ok": True}
...
async def status(session_id: str):
    ...
    running = bool(proc and proc.poll() is None)
    last = LAST_INGEST_AT.get(session_id)
    last_sec = (datetime.utcnow() - last).total_seconds() if last else None
    sockets = len(SESSION_SOCKETS.get(session_id) or [])
    m = SESSION_METRICS.get(session_id, {})
    return {
        "capture": "running" if running else "stopped",
        "lastIngestSec": last_sec,
        "sockets": sockets,
        "metrics": {
            "asr_ms": m.get("asr_ms", 0.0),
            "mt_ms": m.get("mt_ms", 0.0),
...
# ---------- capture ----------
async def _start_capture(session_id: str, source: str = "auto") -> int:
    proc = CAPTURE_PROC.get(session_id)
...
async def start_capture(payload: dict = Body(...)):
    ...
    if not session_id:
        raise HTTPException(status_code=400, detail="sessionId required")
    pid = await _start_capture(session_id, source)
...
async def ingest_segment(payload: dict = Body(...)):
    ...
    try:
        ...
        if kind == "partial":
            ...
            asr_ms = float(payload.get("asrMs", 0.0) or 0.0)
            if asr_ms:
                _update_metrics(session_id, asr_ms=asr_ms)
            try:
                if text_src:
                    ...
                    _update_metrics(session_id, mt_ms=mt_ms)
                else:
                    seg["textEnPartial"] = ""
            except Exception:
