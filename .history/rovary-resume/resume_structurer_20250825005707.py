...
/*************  ✨ Windsurf Command ⭐  *************/
    """
    Call local llama-server to generate structured JSON from a resume.

    :param messages: Chat messages to pass to the LLM (system + user prompts)
    :return: Valid JSON output (or raises LLMError on issue)
    """
/*******  3baa95a6-fc50-408e-b7cb-211992d1902c  *******/
    payload = {
    "model": MODEL_NAME,
    "messages": messages,
    ...
    try:
        resp = requests.post(LLAMA_API_URL, json=payload, timeout=(10,180))
    except Exception as e:
