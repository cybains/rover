...
def ingest_country(country_code: str):
    indicators = ["SP.POP.TOTL", "NY.GDP.MKTP.CD", "NY.GDP.PCAP.CD", "SP.DYN.LE00.IN", "SE.ADT.LITR.ZS"]

    print(f"Testing with {len(indicators)} indicators for {country_code}")
    ...
    for ind in indicators:
        try:
            ...
            print(f"Indicator {ind} produced {len(docs)} normalized docs")

            if docs:
                mongo_docs = []
                for d in docs:
                    mongo_docs.append({
                        "core": d.core.__dict__ if hasattr(d.core, "__dict__") else d.core,
                        "facet": d.facet,
                        "raw": d.raw
                    })

                print(f"Inserting {len(mongo_docs)} docs for {country_code} - {ind}")
                collection.insert_many(mongo_docs, ordered=False)
            else:
                print(f"No docs for {country_code} - {ind}")
