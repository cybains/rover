...
def get_real_country_ids(sess: requests.Session) -> Set[str]:
    ...
    for rows in paged(sess, endpoint):
        for c in rows:
            ...
            cid = c.get("id")
            name = c.get("name")
            if region_id and region_id != "NA" and cid:
                keep.add(cid)
                # upsert minimal country meta
...
def process_indicator(ind_id: str, countries_keep: Set[str]) -> int:
    ...
    for rows in paged(sess, endpoint):
        for r in rows:
            ...
            c = r.get("country") or {}
            cid = c.get("id") or ""
            if COUNTRIES_ONLY and cid and cid not in countries_keep:
                continue
            date = str(r.get("date") or "")
            val  = r.get("value")
            doc = {
                "_id": f"{ind_id}|{cid}|{date}",
                "indicator": {"id": ind_id, "name": (r.get("indicator") or {}).get("value")},
                "country":   {"id": cid, "iso3": r.get("countryiso3code") or cid, "name": c.get("value")},
                "date": date,
                "value": val,
                "unit": r.get("unit"),
    ...
    # minimal indicator meta
    ind_name = None
    # fetch one row name if empty
    if not ind_name:
        col_indicators.update_one(
            {"id": ind_id},
            {"$setOnInsert": {"id": ind_id}},
            upsert=True
        )
...
def main():
    ...
    # Indexes (small dataset; safe to create now)
