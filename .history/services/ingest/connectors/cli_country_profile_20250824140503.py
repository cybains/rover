...
def print_profile(b: Dict[str, Any], limit: int):
    ...
    if limit and len(heads) > limit:
        ...
        # skip entries with explicit no_data status
    rows = [h for h in heads if h.get("status") != "no_data"]

    print(f"{'KPI (code)':<28} {'Year':<10} {'Value':>14} {'YoY':>12} {'Pctl W/R/I':>16}")
    print("-" * 80)
    for h in rows:
        code = h.get("code", "—")
        y = h.get("year")
        # add lag badge if headline year < bundle year
        if isinstance(y, int) and isinstance(year, int) and y < year:
            y_str = f"{y} (-{year - y}y)"
        else:
        ...
        pstr = "—/—/—" if (pw, pr, pi) == (None, None, None) else \
               f"{'' if pw is None else int(round(pw))}/" \
               f"{'' if pr is None else int(round(pr))}/" \
               f"{'' if pi is None else int(round(pi))}"

        print(f"{code:<28} {y_str:<10} {val:>14} {yoy:>12} {pstr:>16}")
