            ...
            for i, y in enumerate(years_sorted):
                ...
                if i > 0:
                    ...
                    if vprev is not None:
                        yoy_abs = float(v_clip - min(max(vprev, lo if not math.isnan(lo) else vprev,
                                                        hi if not math.isnan(hi) else vprev), 
                                                    hi if not math.isnan(hi) else vprev))
                        # percent growth if both positive
                        if vprev > 0 and v_clip > 0:
                            yoy_pct = float((v_clip / vprev - 1.0) * 100.0)
