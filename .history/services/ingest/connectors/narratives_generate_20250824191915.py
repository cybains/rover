# services/ingest/connectors/narratives_generate.py
...
NARRATIVE_VERSION = os.getenv("NARRATIVE_VERSION", "v2").lower().strip()
...
# -------------------- Helpers --------------------
def local_snapshot_date() -> str:
    """Return today's date in Europe/Vienna as YYYY-MM-DD."""
    try:
        tz = ZoneInfo("Europe/Vienna") if ZoneInfo else None
    except Exception:
        tz = None
...
def make_template(bundle: Dict[str,Any], facts: Dict[str,Any], version: str = "v2") -> Dict[str,Any]:
    ...
    return {
        ...
        "country_name": bundle["country"]["name"],   # <- helpful for UI titles
        "year": y,
...
def pick_pctl(bundle: Dict[str, Any], code: str) -> Dict[str, Optional[float]]:
    for h in bundle.get("headlines", []):
        if h.get("code") == code and h.get("status") != "no_data":
            p = h.get("pctl") or {}
            return {
...
def make_callouts(bundle: Dict[str, Any], codes: List[str]) -> Dict[str, List[Dict[str,str]]]:
    ...
    for code in codes:
        ...
        if code in LOWER_IS_BETTER:
            if p <= 25:
                strengths.append({"code": code, "label": f"Favourable {title_case(label)}"})
            elif p >= 75:
                watchouts.append({"code": code, "label": f"High {title_case(label)}"})
        else:
            if p >= 75:
                strengths.append({"code": code, "label": f"Strong {title_case(label)"})_
            elif p <= 25:
...
def build_narrative(bundle: Dict[str,Any], narrative_version: str) -> Dict[str,Any]:
    ...
    try:
        ...
        nar = {
            "iso3": bundle["country"]["id"],
            "country_name": bundle["country"]["name"],
            "year": bundle["year"],
            ...
            "generator": {"mode": "llamacpp", "prompt_version": PROMPT_VERSION, "narrative_version": narrative_version}
        }
    except Exception as e:
        print(f"[warn] LLM generation failed: {e} -> using template")
        nar = make_template(bundle, facts, narrative_version)

    # augment: lag_years, percentiles, callouts, source_links, snapshot_date
    nar["generator"]["narrative_version"] = narrative_version
    nar["snapshot_date"] = local_snapshot_date()
    ...
    for key, f in facts.items():
        ...
        lag = None
        try:
            if isinstance(bundle_year, int) and isinstance(yr, int):
                lag = max(0, bundle_year - yr)
        except Exception:
            pass
        facts_used.append({
            "code": code,
            ...
            "value": f.get("value"),
            "lag_years": lag,
            "pctl": pick_pctl(bundle, code)
        })
        codes_for_callouts.append(code)
        links[code] = source_link(code)
