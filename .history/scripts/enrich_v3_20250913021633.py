...
# --- SKILLS coverage (append to loaded sets for immediate effect) ---
SKILLS_HR_TS_LIST |= set([
    "hubspot","seo","sem","cms","wordpress","webflow","google analytics","copywriting",
    "social media","linkedin","x","email marketing","figma","jira","g suite","slack"
])

SKILLS_TECH_LIST |= set([
    "node.js","node","typescript","javascript","postgres","graphql","rest","api","api gateway","retool",
    "terraform","helm","ansible","prometheus","grafana","elk","elastic","kibana","iam","oauth2","jwt","prefect","airflow"
])
...
def derive_remote(text: str, provider: Dict[str, Any]) -> Tuple[bool, str, List[str]]:
    ...
    # Strong patterns
    if re.search(r"\b(remote[- ]first|fully\s+remote|work\s+from\s+home|anywhere)\b", t):
        remote, mode = True, "remote"

    # "can be based remotely in X or Y"
    m = re.search(r"\b(can\s+be\s+based\s+remotely\s+in|remote\s+in)\s+([^.]+)", t)
    if m:
        remote, mode = True, "remote"
        area = m.group(2)
        # split by commas / or
        parts = re.split(r"\s*(?:,| or | and )\s*", area, flags=re.I)
        norm = []
        for p in parts:
            p = p.strip(" .")
            if not p: continue
            if re.search(r"\b(us|u\.?s\.?a?\.?|united states)\b", p, re.I):
                norm.append("United States")
            elif re.search(r"\beurope|eu\b", p, re.I):
                norm.append("Europe")
            else:
                norm.append(p)
        regions = sorted(list(dict.fromkeys(norm)))

    # Provider hint
    loc = (provider or {}).get("candidate_required_location")
    if loc and not regions:
        regions = [loc.strip()]

    # Hybrid / onsite override if explicitly mentioned
    if re.search(r"\bhybrid\b|\b\d+\s+days\s+onsite\b|\b\d{1,3}%\s+onsite\b", t):
        remote, mode = True, "hybrid"
    if re.search(r"\bonsite\b|\bon[- ]site\b", t) and not re.search(r"\bremote\b", t):
...
LANG_MAP = {
    ...
    "italian":"it","italiano":"it",  # NOTE: no bare "it"
...
def parse_languages(text: str) -> List[Dict[str, Any]]:
    ...
    # Plain language mention
    for lang_name, code in LANG_MAP.items():
        if re.search(rf"\b{lang_name}\b", t, re.I):
            if not any(d["lang"]==code for d in out):
                out.append({"lang": code, "level": None, "source": "desc", "confidence": 0.6})
    return out[:6]
...
BENEFIT_VOCAB = {
    ...
    "mentorship":"Mentorship", "bonus":"Bonus"
}

def normalize_benefits(raw_bullets: List[str], full_text: str = "") -> List[str]:
    """
    Normalize both from bullets and the full JD text.
    """
    s = " | ".join(raw_bullets + [full_text]).lower()
    out = set()
...
def adjust_seniority(sen: str, yrs: Optional[int], title_hint: str = "") -> str:
    """
    If a range like 2–5 is detected upstream (we only get a single int),
    prefer 'mid' unless title explicitly says 'Senior' and min >=5.
    """
    t = title_hint.lower()
    if not sen and yrs is not None:
        if yrs >= 5 and ("senior" in t or "sr." in t):
            return "senior"
        if yrs >= 2:
            return "mid"
        return "junior"
...
def qa_flags_and_score(jd: Dict[str, Any]) -> Tuple[List[str], int]:
    ...
    resp = jd.get("responsibilitiesList") or []
    reqs = jd.get("requirementsList") or jd.get("requirements") or []
    if len(resp) < 3: flags.append("RESP_TOO_SHORT")

    # Headers accidentally included?
    header_tokens = ("what you’ll do","what you will do","why join","key responsibilities","requirements","qualifications","location")
    joined_resp = " | ".join(resp).lower()
    joined_reqs = " | ".join(reqs).lower()
    if any(h in joined_resp for h in header_tokens):
        flags.append("RESP_CONTAINS_HEADERS")
    if any(h in joined_reqs for h in ("why join","benefits","perks")):
    ...
    ct = jd.get("compensationText", "") or ""
    cp = jd.get("compensationParsed") or {}
    if cp and (cp.get("min") or cp.get("max")):
        if re.search(r"\b(yrs?|years?)\b", ct.lower()):
            flags.append("SALARY_CONTEXT_MISMATCH")
    else:
        # salary mentioned but not parsed
        if re.search(r"\b(\$|€|£|usd|eur|gbp).{0,12}\d", (jd.get("keywordsTaxonomy") and " ".join(jd["keywordsTaxonomy"]) or "") + " " + ct, re.I):
            flags.append("SALARY_PARSE_FAIL")

    # Location noise: >7 items and less than half are in gazetteer
    locs = jd.get("locationMentions") or []
    if len(locs) >= 7:
        gaz_count = sum(1 for x in locs if x.lower() in COUNTRIES or x.lower() in CITIES_HINT or x in {"United States","United Kingdom"})
        if gaz_count < len(locs) / 2:
            flags.append("LOCATION_NOISE")

    # Skills sanity for certain categories
    tools = set((jd.get("toolsTechnologies") or []))
    rc = (jd.get("roleCategory") or "").lower()
    if "marketing" in rc:
        expected = {"hubspot","seo","cms","social media","email marketing"}
        if len(tools & expected) == 0:
            flags.append("SKILLS_KEYWORD_MISS")
    if "infrastructure" in rc or "devops" in rc:
        expected = {"kubernetes","terraform","helm","prometheus","grafana"}
        if len(tools & expected) <= 1:
            flags.append("SKILLS_KEYWORD_MISS")

    score = 100
    penalties = {
        "MISSING_TITLE":15, "MISSING_SENIORITY":10, "RESP_TOO_SHORT":10,
        "RESP_CONTAINS_HEADERS":8, "REQS_CONTAINS_BENEFITS":6,
        "SENIORITY_INCONSISTENT":10, "REMOTE_ENUM_INVALID":10,
        "SALARY_CONTEXT_MISMATCH":12, "SALARY_PARSE_FAIL":8,
        "LOCATION_NOISE":6, "SKILLS_KEYWORD_MISS":8
    }
    for fl in flags:
        score -= penalties.get(fl, 5)
...
def enrich_one(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    jd["seniorityLevel"] = adjust_seniority(jd["seniorityLevel"] or "", yrs, job.get("title") or "")
    ...
    COUNTRIES = {
    "united states","usa","us","europe","philippines","serbia","hungary","portugal","netherlands","germany","france","spain","italy","austria","uk","united kingdom","canada"
}
CITIES_HINT = {"belgrade","budapest","manila","lisbon","amsterdam","berlin","paris","madrid","rome","london","vienna","boston","new york","san francisco","los angeles"}

def _parse_location_label(text: str) -> List[str]:
    m = re.search(r"^\s*location\s*[:\-]\s*(.+)$", text, re.I | re.M)
    if not m:
        return []
    rhs = m.group(1)
    parts = re.split(r"\s*(?:,| or | and )\s*", rhs)
    return [p.strip(" .") for p in parts if p.strip()]

def parse_locations(text: str) -> List[str]:
    out = set()
    # 1) From explicit label
    for item in _parse_location_label(text):
        out.add(item)
    # 2) From gazetteer scan
    tl = text.lower()
    for c in COUNTRIES:
        if re.search(rf"\b{re.escape(c)}\b", tl):
            norm = {
                "us": "United States", "usa": "United States"
            }.get(c, c.title() if c != "uk" else "United Kingdom")
            out.add(norm)
    for city in CITIES_HINT:
        if re.search(rf"\b{re.escape(city)}\b", tl):
            out.add(city.title())
    return sorted(list(out))[:10]

    jd["locationMentions"] = parse_locations(cleaned)


    # work hours overlap
    m_wh = re.search(r"\b(\d+)\s*hours?\s+of\s+overlap\s+with\s+([A-Z]{2,4})\b.*?\b(\d{1,2}\s*(?:am|pm))\s*to\s*(\d{1,2}\s*(?:am|pm))\s*\2", cleaned, re.I)
    if m_wh:
        jd["workHoursWindow"] = f"{m_wh.group(1)} hours overlap with {m_wh.group(2)} ({m_wh.group(3)}–{m_wh.group(4)} {m_wh.group(2)})"
    else:
        # simpler: at least capture "overlap with PST (8 am to 2 pm PST)"
        m_wh2 = re.search(r"\boverlap\s+with\s+([A-Z]{2,4}).*?\(([^)]+)\)", cleaned, re.I)
        if m_wh2:
            jd["workHoursWindow"] = f"Overlap with {m_wh2.group(1)} ({m_wh2.group(2)})"

    # on-call rotation
    if re.search(r"\bon-?call\b.*\brotation\b", cleaned, re.I):
        jd["workSchedule"] = "On-call rotation"
    ...
    for pat, label in [
        ...
        if re.search(pat, cleaned, re.I):
            comp_hits.add(label)
    if comp_hits:
        jd["complianceRegulations"] = sorted(list(comp_hits))
