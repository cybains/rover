...
def extract_role_category_and_specialty(title: str, text: str) -> Tuple[str, str]:
    ...
    if re.search(r"\b(logistics|transport(ation)?|fleet)\b", t):
...
def parse_trucking_schedule(text: str) -> Tuple[Optional[str], Optional[str]]:
    """
    Returns (workSchedule, travelNote)
    workSchedule examples: 'OTR', 'Regional', 'Local', 'Night Shift', 'Weekend Shift', 'Home Daily'
    travelNote free-text hints like 'home weekly'
    """
    t = text.lower()
    ws = []
    note = None
    ...
    if ws:
        return (", ".join(sorted(set(ws))), note)
    return (None, note)
...
def pick_allowlist(role_category: str) -> set:
    ...
    # fallback: very small union to avoid hallucinations
    return SKILLS_TECH_LIST  # safe default; expand later if needed
