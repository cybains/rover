...
_TAIL_S = int(0.5 * _SR)       # 500 ms overlap tail for smoother joins
...
def _init_model() -> WhisperModel:
    ...
    Env-tunable model:
      ...
      ASR_DEVICE     (default: "cpu"  | set "cuda" for GPU)
      ASR_COMPUTE    (default: "int8" | "float16" for GPU)
      ASR_FORCE_LANG (optional: e.g., "de")
    """
...
def _load_model():
    global _MODEL
    _MODEL = _init_model()
    # confirm env (WhisperModel does not expose .device directly)
    print(
        f"[ASR] Model initialized (WhisperModel) "
        f"with env: ASR_DEVICE={os.getenv('ASR_DEVICE')} ASR_COMPUTE={os.getenv('ASR_COMPUTE')}"
    )
...
def transcribe_chunk(payload: ChunkIn):
    ...
    Uses 500ms overlap + rolling prompt for stability, and returns segments with
    ...
    # Decode settings: no VAD (avoids onnxruntime), better beam for accuracy
    segments, info = _MODEL.transcribe(
        audio_f32,
        language=lang_arg,                 # None = autodetect; set "de" to force German
        vad_filter=False,                  # keep False to avoid onnxruntime
        beam_size=5,                       # a bit more accurate than 3
        best_of=1,
        temperature=0.0,
