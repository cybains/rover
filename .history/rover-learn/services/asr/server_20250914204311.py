...
def _load_model():
    global _MODEL
    _MODEL = _init_model()
    # safer way: just confirm type
    print(f"[ASR] Model initialized ({type(_MODEL).__name__}) with env: "
          f"ASR_DEVICE={os.getenv('ASR_DEVICE')} "
          f"ASR_COMPUTE={os.getenv('ASR_COMPUTE')}")
...
def transcribe_chunk(payload: ChunkIn):
    ...
    segments, info = _MODEL.transcribe(
        ...
        vad_filter=False,                   # ignore silence
